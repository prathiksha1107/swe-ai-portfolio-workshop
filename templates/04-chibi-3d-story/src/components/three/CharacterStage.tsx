...
export function CharacterStage({ activeSection, reducedMotion }: { activeSection: string; reducedMotion: boolean }) {
  ...
  const stageVisible = characterSection === "hero" || characterSection === "contact" || (!coarsePointer && characterSection === "about");
  const stageStyle = { "--contact-stage-top": `${contactTop}px` } as CSSProperties;
  return (
    <div ref={stageElement} style={stageStyle} className={`character-stage ${stageVisible ? "is-visible" : "is-hidden"}${characterSection === "hero" ? " is-interactive" : ""}${characterSection === "contact" ? " is-contact" : ""}`}>
      <div className="character-stage__halo" />
      {characterSection === "hero" && !coarsePointer && <ul className="character-tech-cloud" aria-label="Technologies used in current work">{profile.experience[0].technologies.map((technology) => <li key={technology}><span>{technology}</span></li>)}</ul>}
